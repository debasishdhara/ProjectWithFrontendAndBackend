...
const controllersourcePath: string = path.join(__dirname, 'baseStructure', 'workers', 'baseWorker.ts');
const controllerdestinationPath: string = path.join(dirPath, 'app', 'workers', `${(routerName).toLowerCase()}Worker.ts`);
