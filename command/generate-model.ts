...
if ((routerName).toLocaleLowerCase() === 'default' || (routerName).toLocaleLowerCase() === 'pbase' || (routerName).toLocaleLowerCase() === 'mbase' || (routerName).toLocaleLowerCase() === 'sbase') {
  console.error('❌ Please provide a model name other than "default", "pbase", "mbase", or "sbase".');
  process.exit(1);
}
...
const controllersourcePath: string = path.join(__dirname, 'baseStructure', 'models', 'base.ts');
const controllerdestinationPath: string = path.join(dirPath, 'app', 'model', `${toSentenceCase(routerName)}.ts`);

(async () => {
  if (fs.existsSync(controllerdestinationPath)) {
    console.log(`⚠️ Model already exists at ${controllerdestinationPath}. Skipping creation.`);
  } else {
    const resultOfController = await createFile(controllersourcePath, controllerdestinationPath, routerName);
    console.log(`✅ Model created at ${controllerdestinationPath}`);
  }
})();
